...
let config = {
    ...
    POLLING_GROUP_ID: '', // ID вашей группы
    ...
    ADMIN_ID: INT // Ссылка на вашу страницу
}
```
...
|**```vk.js```**|```./src/index.js```|В данном файле хранится конструктор __VK__, а также обработчик сообщений и клавиатуры. Этот файл подключаем ко всем побочным файлам, подразумевающим обработку разных команд.|
|**```links.js```**|```./src/links.js```| В данном файле хранится информация о предметах и ссылках на зум. 
|**```automasticSender.js```**|```./src/automaticSender.js```|
...
## Команды
   ...
   7. ```/шпора ?``` - инструкция по шпорам
   8. ```/шпора список``` - список шпор
   9. ```/citgen``` - перешлите чье-то сообщение и напишите эту команду
   10. ```/help``` - документация бота
